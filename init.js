      ...
      let daily = [];
        ...
        for(let date in daily){
          ...
          if(newDate >= start2 && newDate <= end2){
            ...
            parseFloat(daily[date]['3. low']),
             parseFloat(daily[date]['2. high']),
...
function completeFn(results) {
  ...
  for(var i = 1;i<csv.length;i++){
    if(!isNaN(csv[i][indeces['open']]) && !isNaN(csv[i][indeces['close']]) && !isNaN(csv[i][indeces['low']]) && !isNaN(csv[i][indeces['high']]) && !isNaN(csv[i][indeces['volume']])){
      stocks.push([parseFloat(parseFloat(csv[i][indeces['open']]).toFixed(2)),
      parseFloat(parseFloat(csv[i][indeces['close']]).toFixed(2)),
      parseFloat(parseFloat(csv[i][indeces['low']]).toFixed(2)),
      parseFloat(parseFloat(csv[i][indeces['high']]).toFixed(2))]);
...
//     ]
//   };
//   var bar_plot = echarts.init(document.getElementById('div_dist'),'dark');
...
function plot_stock(){
  ...
  $('#graphAlt').hide(); 
  $('#div_output').attr('style','height:500px');
